...
export default function SettingsScreen({ onBack }: SettingsScreenProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // Privacy
      const { data: privacyData } = await supabase.from('user_privacy').select('*').eq('id', user.id).single();
    ...
    const privacySub = supabase.channel(`user_privacy_${user.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'user_privacy', filter: `id=eq.${user.id}` }, (payload) => {
  ...
  const updateSettingsData = async (data: any) => {
    ...
    try {
      ...
      const { error } = await supabase.from('user_settings').upsert({ ...newSettings, id: user.id });
      if (error) throw error;
    } catch (error: any) {
      showFeedback(error.message, 'error');
    }
  ...
  const updatePrivacyData = async (data: any) => {
    if (!user) return;
    try {
      const { error } = await supabase.from('user_privacy').upsert({ ...privacy, ...data, id: user.id });
      if (error) throw error;
    } catch (error: any) {
      showFeedback(error.message, 'error');
    }
      ...
      <div className="max-w-2xl mx-auto mt-2 space-y-0 px-0">
        ...
        <div className="bg-white border-b border-zinc-100">
          ...
          <div className="divide-y divide-zinc-50">
            <button 
              onClick={() => {
                const name = window.prompt("Novo nome:", profile?.name);
                if (name) updateProfileData({ name: name });
              }}
